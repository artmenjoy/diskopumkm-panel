...
class C_berita extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('content/table_berita');
    ...
    public function index()
    {
		...
		$no=0;
		foreach ($data as $d) {
			$post[$no]['post_category_id'] 	= $d->id_kategori;;
			$post[$no]['title'] 	= $d->judul;
			$post[$no]['slug'] 		= strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $d->judul)));
			$post[$no]['date']      = date("Y-m-d",strtotime($d->tanggal));
			$post[$no]['datetime']  = strtotime($d->tanggal." ".$d->jam);
			if($d->status == "Y"){
				$post[$no]['status']    = 1;
			}else{
				$post[$no]['status']    = 0;
			}
			$post[$no]['thumbnail'] = $d->gambar;
			$post[$no]['content']   = $d->isi_post;
			$post[$no]['user_id']   = 1;
			$post[$no]['view'] 		= $d->dibaca;
			$post[$no]['edited'] 	= strtotime($d->tanggal." ".$d->jam);
			$no++;
		}

		$this->table_post->add_batch($post);
